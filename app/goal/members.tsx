...
export default function GoalMembersScreen() {
  const params = useLocalSearchParams<{ goalId: string; goalName?: string }>();
  const goalId = params.goalId;

  const { data: goalData, isLoading: goalLoading } = useGoal(goalId);
  const removeMemberMutation = useRemoveMember();
  const addMemberMutation = useShareGoal();
  ...
  const members = (goalData?.data?.others_member || []) as Member[];
