import { useGoalSummary, useBookOverview } from "@/api/statistics";
...
export default function StatisticsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"wallet" | "goal">("wallet");

  const { data: overview, isLoading } = useBookOverview({
    period: period === "all" || period === "custom" ? undefined : period,
    from_date: fromDate?.toISOString().split("T")[0],
    to_date: toDate?.toISOString().split("T")[0],
  });
  const { data: goalSummary, isLoading: isGoalLoading } = useGoalSummary();
    ...
    <ScreenContainer className="bg-background">
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
        ...
        <View className="px-6 mb-6">
          <View className="flex-row bg-muted/30 p-1.5 rounded-[28px] border border-border/50">
            <Pressable
              onPress={() => setActiveTab("wallet")}
              className={cn(
                "flex-1 py-3.5 rounded-[22px] items-center justify-center flex-row gap-x-2",
                activeTab === "wallet"
            ...
            >
              <WalletIcon
                size={18}
                color={activeTab === "wallet" ? "white" : "rgb(115, 115, 115)"}
              />
              <P
                className={cn(
                  "font-bold",
            ...
            >
              <GoalIcon
                size={18}
                color={activeTab === "goal" ? "white" : "rgb(115, 115, 115)"}
              />
              <P
                className={cn(
                  "font-bold",
                    ...
                    >
                      <View className="flex-row items-center gap-x-3">
                        <Filter
                          size={18}
                          color={
                            period !== "custom" ? "white" : "rgb(115, 115, 115)"
                          }
                        />
                        <P
                          className={cn(
          ...
          <View className="px-6 pb-12">
            {isGoalLoading ? (
              ...
              <View className="gap-y-6">
                {/* Goal Overview Card */}
                <View className="bg-primary p-8 rounded-[40px] shadow-2xl shadow-primary/40 mb-4 overflow-hidden relative">
                  <View className="absolute -top-10 -right-10 w-40 h-40 bg-white/10 rounded-full" />
                  <View className="absolute -bottom-10 -left-10 w-24 h-24 bg-black/10 rounded-full" />
                  ...
                  <H1 className="text-primary-foreground text-5xl mb-6">
                    {Math.round(
                      ((goalSummary?.data?.total_saved || 0) /
                        (goalSummary?.data?.total_target || 1)) *
                        100,
                    )}
                    %
                  </H1>
                  ...
                  <View className="flex-row items-center gap-x-4">
                    ...
                    <View className="flex-1 bg-black/10 p-4 rounded-3xl backdrop-blur-md">
                      ...
                      <P className="text-primary-foreground font-bold text-lg">
                        $
                        {Number(
                          goalSummary?.data?.total_target || 0,
                        ).toLocaleString()}
                      </P>
                    </View>
                  </View>
                ...
                {/* Individual Goals */}
                <View className="gap-y-6">
                  {goalSummary?.data?.goals?.map((goal: any) => {
                    const progress =
                    ...
                    return (
                      ...
                      >
                        ...
                        <View className="flex-row justify-between mt-4">
                          <Muted className="text-[10px] font-bold uppercase">
                            Remaining
                          </Muted>
